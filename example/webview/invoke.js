var MpInvoke = (function () {
  ...
  function Messager(senderHandle) {
    ...
    var fn = {}; //对方提供的函数
    var transactions = {}; //事务集
    var callbacks = {}; //回调
    var needWait = []; //需要等待的事务
    var EE = new events(); //事件对象
    ...
    function sender(model) {
      var force = model.command === SYNC_COMMAND; // force send the message when the message is the sync message
    ...
    function listener(data) {
      var model = new MsgBody(data);
      if (model.reply) {
        var key = getTransactionKey(model);
        ...
        if (callbacks[model.command]) {
          var result = callbacks[model.command](model.data);
          console.log("run-callback", result);
          if (result && result.then) {
            result.then(function (rs) {
              return reply(model, rs);
            });
          } else {
            reply(model, result);
          }
     ...
     */
    function sync() {
      __sync(Object.keys(callbacks)).then(function (res) {
        console.log("---", res);
        return _sync(res);
      });
    }
  ...
  var _ref = new Messager(function (data) {
  ...
  if (isBrowser) {
    var originalPostMessage = window.my;
    if (originalPostMessage) {
      console.log("do-ready");
      ready();
      if (!originalPostMessage.onMessage) {
        originalPostMessage.onMessage = function (data) {
  ...
  var invoker = /*#__PURE__*/Object.freeze({
    default: browser
  });
